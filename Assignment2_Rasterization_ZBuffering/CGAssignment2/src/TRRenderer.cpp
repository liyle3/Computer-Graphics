...
namespace TinyRenderer
{
	...
	void cliping_function(
          ...
          for (int i = 0; i < num; i++) {
            ...
            if (flag[from] * flag[to] < 0) {
              ...
              if (plane == 0) {
                    ...
                    ((vertexs[from].cpos.w - signal * vertexs[from].cpos[plane]) - (vertexs[to].cpos.w - signal * vertexs[to].cpos[plane]));
              }
              TRShaderPipeline::VertexData node = TRShaderPipeline::VertexData::lerp(vertexs[from], vertexs[to], t);
        ...
        std::vector<TRShaderPipeline::VertexData> TRRenderer::cliping(
          ...
          std::vector<TRShaderPipeline::VertexData> vertexs, arr_1, arr_2, arr_3, arr1, arr2, arr3, ans;
