...
namespace TinyRenderer
{
	...
	void TRPhongShadingPipeline::fragmentShader(const VertexData &data, glm::vec4 &fragColor)
	{
		...
		for (size_t i = 0; i < m_point_lights.size(); ++i)
		{
			...
			{
				double d = sqrt(pow(fragPos.x - light.lightPos.x, 2) + 
					            pow(fragPos.y - light.lightPos.y, 2) + 
					            pow(fragPos.z - light.lightPos.z, 2));

				attenuation = 1.0f / (light.attenuation.x + light.attenuation.y * d + light.attenuation.z * d * d);
				normal = glm::normalize(normal);
				viewDir = glm::normalize(-viewDir);
				glm::vec3 reflect = glm::normalize(glm::reflect(lightDir, normal));
				lightDir = glm::normalize(-lightDir);
				glm::vec3 h = glm::normalize(viewDir + lightDir) / glm::length(viewDir + lightDir);
				ambient = light.lightColor * amb_color; 
				diffuse = m_kd * light.lightColor * dif_color * glm::max(glm::dot(normal, lightDir), 0.0f);
				specular = m_ks * light.lightColor * spe_color * pow(glm::max(glm::dot(reflect, viewDir), 0.0f), m_shininess);
				//specular = m_ks * light.lightColor * spe_color * pow(glm::max(glm::dot(normal, h), 0.0f), m_shininess);
